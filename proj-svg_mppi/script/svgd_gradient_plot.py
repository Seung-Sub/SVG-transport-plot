...
# 이제 row가 전부 0이므로, "Iteration, SampleIdx, Col" 그룹별 평균을 구하면
# 실제로는 t=0에 해당하는 값만 집계됨
grouped = df.groupby(["Iteration", "SampleIdx", "Col"], as_index=False)["GradValue"].mean()
...
for ax, sample_idx in zip(axes, unique_particles):
    ...
    # col=0,1,2 => Vx, Vy, Wz (가정)
    pivot_df.plot(ax=ax, marker='o')

    ax.set_title(f"Guide Particle #{sample_idx} - (Row=0 기준)")
    ax.set_xlabel("Iteration")
    ax.set_ylabel("Mean Gradient Value at Row=0")
    ax.grid(True)

plt.tight_layout()
plt.savefig("svgd_gradient_eachparticle_t0.png")
plt.show()