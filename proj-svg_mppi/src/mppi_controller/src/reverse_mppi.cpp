...
namespace cpu {
    ReverseMPPI::ReverseMPPI(const Params::Common& common_params, const Params::ReverseMPPI& reverse_mppi_params)
        : prediction_step_size_(static_cast<size_t>(common_params.prediction_step_size)),
          ...
          Vx_cov_(reverse_mppi_params.Vx_cov)
        //   Vy_cov_(reverse_mppi_params.Vy_cov),
        //   Wz_cov_(reverse_mppi_params.Wz_cov) 
          {
...
#pragma omp parallel for num_threads(thread_num_)
    ...
    // void ReverseMPPI::set_obstacle_map(const nav_msgs::OccupancyGrid& obstacle_map) { mpc_base_ptr_->set_obstacle_map(obstacle_map); };

    void ReverseMPPI::set_reference_path(const nav_msgs::Path& reference_path) { mpc_base_ptr_->set_reference_path(reference_path); };

    void ReverseMPPI::set_single_goal(const geometry_msgs::PoseStamped& goal) { mpc_base_ptr_->set_single_goal(goal); };
